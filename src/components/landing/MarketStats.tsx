...
export const MarketStats = () => {
  ...
  const marketCap = useCountUp(
    marketData ? Math.floor(marketData.total_market_cap / 1e9) : 0,
    2000,
    0,
    isVisible
  );
  ...
  const volume = useCountUp(
    marketData ? Math.floor(marketData.total_volume / 1e9) : 0,
    2000,
    0,
    isVisible
  );
  ...
  return (
    <section ref={ref} className="py-16 bg-gradient-to-b from-background via-muted/20 to-background relative overflow-hidden">
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          <div className={`glass-card p-6 rounded-xl hover-lift animate-optimized transition-all duration-300 group ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
            <div className="flex items-center justify-between mb-4">
              ...
              {marketData && (
                <span className={`text-sm font-medium ${marketData.market_cap_change_percentage_24h >= 0 ? 'text-green-500' : 'text-red-500'}`}>
